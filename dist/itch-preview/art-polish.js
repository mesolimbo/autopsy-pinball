...
for(const kind of ['brain','stomach','bone','mouth']){const img=new Image();img.src=GAME_TEXTURES['assets/'+kind+'-material.png'];organMaterials[kind]=img;}
const footImage=new Image();footImage.src=GAME_TEXTURES['assets/foot-sprite-green.png'];
const footMaskImage=new Image();footMaskImage.src=GAME_TEXTURES['assets/foot-sprite.png'];
let greenFootSurface;
...
function drawFoot(f){
  ...
  if(footImage.complete&&footImage.naturalWidth&&footMaskImage.complete&&footMaskImage.naturalWidth){
    ctx.strokeStyle='#6c7960';ctx.lineWidth=2;ctx.stroke(footShape.path);
    if(!greenFootSurface){greenFootSurface=document.createElement('canvas');greenFootSurface.width=560;greenFootSurface.height=240;const c=greenFootSurface.getContext('2d');c.drawImage(footImage,40,0,2076,729,0,0,560,240);c.globalCompositeOperation='destination-in';c.drawImage(footMaskImage,...FOOT_IMAGE_BOUNDS,0,0,560,240);}
    ctx.drawImage(greenFootSurface,-14,-22,112,48);
